...
import com.watirna.shop.databinding.ActivityEditCategoryBinding
...
class EditCategoryActivity : BaseActivity<ActivityEditCategoryBinding>(), EditCategoryNavigator {

    lateinit var viewModel: EditCategoryViewModel
    lateinit var editCategoryBinding: ActivityEditCategoryBinding
    var keyValue = ArrayList<KeyValuePair>()
    private var mCropImageUri: Uri? = null


    override fun initView(mViewDataBinding: ViewDataBinding?) {
        editCategoryBinding = mViewDataBinding as ActivityEditCategoryBinding
        viewModel = ViewModelProviders.of(this).get(EditCategoryViewModel::class.java)
        editCategoryBinding.editCategoryViewModel = viewModel
        ...
        if(intent.hasExtra("id")) {
            viewModel.getSingleCategory(intent.getIntExtra("id",0))
        }
        ...
        viewModel.singleCategoryResponse.observe(this, Observer {
            loadingObservable.value = false
            viewModel.categoryName.set(it.responseData.store_category_name)
            viewModel.category_desc.set(it.responseData.store_category_description)
           // Toasty.success(this, "" + it.message, Toast.LENGTH_SHORT).show()
        })
    }

    override fun getLayoutId() = R.layout.activity_edit_category
    ...
    private fun setData() {
        ...
        editCategoryBinding.statusSpinner.adapter = adapter
        editCategoryBinding.statusSpinner.setSelection(adapter.getPosition(keyValue.get(0))) //Optional to set the selected item.
    }
        ...
        if (resultCode != Activity.RESULT_CANCELED) {
            ...
            if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
                ...
                if (resultCode == Activity.RESULT_OK) {
                    editCategoryBinding.imageCategory.setImageURI(result.uri)
        ...
        Dexter.withActivity(this)
            ...
            .withListener(object : PermissionListener {
                override fun onPermissionGranted(response: PermissionGrantedResponse?) {
                    CropImage.startPickImageActivity(this@EditCategoryActivity)
                }
                ...
                override fun onPermissionDenied(response: PermissionDeniedResponse?) {
                    //close activity
                    Toasty.error(this@EditCategoryActivity, "Access Denied", Toast.LENGTH_SHORT)
                        .show()
                    //finish()
                }
    ...
    override fun updateCategoryMethod() {
        if (validation()) {
            ...
            hashMap.put(
                ...
                RequestBody.create(
                    "text/plain".toMediaTypeOrNull(),
                    viewModel.category_desc.get().toString()
                )
            )
            hashMap.put(
                ...
                RequestBody.create(
                    "text/plain".toMediaTypeOrNull(),
                    keyValue.get(editCategoryBinding.statusSpinner.selectedItemPosition).key
                )
            )
            ...
            Log.d("update","store_category_name = "+viewModel.categoryName.get().toString()+ " store_category_description = "+viewModel.category_desc.get().toString())
            viewModel.updateCategoryApi(intent.getIntExtra("id",0),hashMap)
